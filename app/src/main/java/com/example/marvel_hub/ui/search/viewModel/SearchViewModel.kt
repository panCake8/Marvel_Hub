...
class SearchViewModel : BaseViewModel() {
    ...
    private val _creators = MutableLiveData<DataState<BaseResponse<CreatorModel>>>(DataState.Loading)
    ...
    private fun onGetComicsSuccess(comics: BaseResponse<ComicModel>) = _comics.postValue(DataState.Success(comics))

    private fun onGetComicsError(throwable: Throwable) = DataState.Error(throwable.message.toString())


    private fun onGetCreatorsSuccess(creators:BaseResponse<CreatorModel>) = _creators.postValue(DataState.Success(creators))
